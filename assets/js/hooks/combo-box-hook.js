...
const ComboBoxHook = {
  mounted() {
    ...
    // Firefox was caching/remembering the shadow select when the user refreshes the page
  ...
  async init(el) {
    ...
    if (remoteOptionsEventName && tomSelectOptions.load) {
      // To prevent this warning, either omit the load configuration in globalOptions
      // or avoid setting remoteOptionsEventName
      console.warn(
        "ComboBox - load has been set by globalOptions, ignoring remoteOptionsEventName",
      );
    } else if (remoteOptionsEventName) {
