defmodule PetalProWeb.DataTable.Header do
  ...
  def render(assigns) do
    ...
    ~H"""
    <.th class={"#{@column[:class] || ""} align-top"}>
      <%= if @column[:sortable] && !@no_results? do %>
        <.a
          class={"#{if @currently_ordered, do: "font-semibold text-gray-900 dark:text-white", else: "text-gray-500 dark:text-gray-400"} #{if @column[:align_right], do: "justify-end", else: ""} flex items-center gap-3"}
      ...
      <% else %>
        <div class={if @column[:align_right], do: "whitespace-nowrap text-right", else: ""}>
          {get_label(@column)}
        </div>
      <% end %>
          ...
          <%= if input_value(f2, :field) == @column.field do %>
            ...
            <div class="mt-2 flex items-center gap-2">
              ...
              <%= if length(@column[:filterable]) > 1 do %>
                <.dropdown>
                  <:trigger_element>
                    <div class="inline-flex w-full items-center justify-center align-middle focus:outline-hidden">
                      <.icon name="hero-funnel" class="h-4 w-4 text-gray-400 dark:text-gray-600" />
                      <.icon
